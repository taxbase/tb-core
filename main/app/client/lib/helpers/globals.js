...
this.locateUser = function () {
  function successFunction(position) {
    ...
    return Meteor.call("tenant/locateAddress", lat, lng, function (error,
      address) {
      if (address) {
  ...
  function errorFunction() {
    return Meteor.call("tenant/locateAddress", function (error, address) {
      if (address) {
        return Session.set("address", address);
      }
    });
...
this.currentBillingAddress = function (addressBooks){
  if (! _.isEmpty(addressBooks)){
    let currentAddress = $.grep(addressBooks, function(e){ return e.isBillingDefault == true; });
    return currentAddress[0];
  }
};

this.currentShippingAddress = function (addressBooks){
  if (! _.isEmpty(addressBooks)){
    let currentAddress = $.grep(addressBooks, function(e){ return e.isShippingDefault == true; });
    return currentAddress[0];
  }
};
...
this.getChatMonthLabel =  function (months) {
  let label = [];
  let allMonths = ["January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December"];
  _.each(months, function (month) {
    label.push(allMonths[month - 1]);
  });
  return label
};
...
Template.registerHelper('formatDateWithHyphens', function(date) {
